...
//			System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
		...
		//if(rchCs.getConstraints().length != 0){
			if(problematicActivity.getSymbolicVariable().getSymbols()[0].contains("at_robot1_manipulationArea")) retPossibleConstraintNetworks.removeElementAt(retPossibleConstraintNetworks.size() - 1);
		//}
